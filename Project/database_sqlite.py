...
order by m."time";"""
SELECT_COUNTER_UNREAD_WITH_SB = """CREATE VIEW IF NOT EXISTS count_unread as select a.nick, (select aa.nick as _from from
account aa
where aa.id_account = (select case 
	when cc.id_receiver = a.id_account then cc.id_sender
	else cc.id_receiver
...
or (c.id_sender = a.id_account and m.receiver_sender = 'from_receiver')) and m.is_read = False)
group by a.nick, _from;
select * from count_unread where _from = ?;"""
